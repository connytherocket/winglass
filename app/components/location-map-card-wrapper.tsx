...
const locations: Location[] = [
  {
    ...
    detailUrl: "/standorte/herrenberg",
  },
  {
    ...
    detailUrl: "/standorte/bietigheim-bissingen",
  },
];
...
export interface LocationMapRef {
  focusLocation: (locationName: string) => void;
...
export const LocationMap = forwardRef<LocationMapRef, LocationMapProps>(
  ({ selectedService }, ref) => {
    ...
    const [markers, setMarkers] = useState<
      Map<string, google.maps.Marker & { infoWindow: google.maps.InfoWindow }>
    >(new Map());
    ...
    useImperativeHandle(ref, () => ({
      focusLocation: (locationName: string) => {
        const markerData = markers.get(locationName);
        if (markerData && map) {
          // Scroll zur Karte
          ...
          // Zoom auf Marker - OHNE InfoWindow zu öffnen
          setTimeout(() => {
            map.setCenter(markerData.getPosition()!);
            map.setZoom(15);
            // InfoWindow wird NICHT geöffnet
          }, 500);
        }
      },
    ...
    useEffect(() => {
      ...
      // Marker und InfoWindows erstellen
      const newMarkers = new Map<
        string,
        google.maps.Marker & { infoWindow: google.maps.InfoWindow }
      >();
      ...
      filteredLocations.forEach((location) => {
        ...
        }) as google.maps.Marker & { infoWindow: google.maps.InfoWindow };

        // InfoWindow für diesen Marker erstellen
      ...
      `;
        ...
        newMarkers.set(location.name, marker);
      });

      setMarkers(newMarkers);
    }, [isLoaded, map, selectedService]);

    // Marker Sichtbarkeit updaten wenn Service-Filter ändert
    useEffect(() => {
      if (!map) return;
      ...
      markers.forEach((marker, locationName) => {
        ...
        const isVisible = selectedService
          ? (location.services?.includes(selectedService) ?? false)
          : true;
    ...
    }, [selectedService, map, markers, filteredLocations]);
...
// Exportiere Locations und Services für externe Nutzung
export { locations, availableServices };
